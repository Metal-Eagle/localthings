...
def test_oven_setpoint_rmw_preserves_other_item_fields():
    ...
    _path, body = desc.write_fn(200, rep)
...
def test_sound_write_is_single_token():
    ...
    _path, body = desc.write_fn("Off", _mode_rep())
...
def test_natural_steam_write_is_single_token():
    ...
    _path, body = desc.write_fn("On", _mode_rep())  # no NaturalSteam_* in rep
...
def test_cook_time_write_produces_hms():
    desc = next(e for e in oven.OVEN_OPERATIONAL_STATE.entities if e.key == "cook_time")
